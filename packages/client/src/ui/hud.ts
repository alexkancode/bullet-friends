...
import { buildSignature, buildSummary } from './build.js'
...
export function updateHud(elements: HudElements, state: GameState, self: PlayerState | undefined, design: GameDesign = defaultDesign()): void {
  ...
  const signature = `${design.name}|${buildSignature(self)}`
  if (signature === renderedBuild) return
  renderedBuild = signature
